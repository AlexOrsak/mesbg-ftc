...
-- Returns the script string for the given tags array, or empty string if no script tag is present.
function M.getScript(tags)
    if not tags then return "" end
    for _, tag in ipairs(tags) do
        if tag == "firingArc" then return SHARED .. FIRING_ARC_EXTRA end
        if tag == "calcBounding" then return SHARED end
    end
    return ""
end

-- Replace with your actual bag's GUID
local MODEL_BAG_GUID = '24f6d0'
local NOTE_NAME = "ArmyList" -- Name of the Notebook tab to read from
...
function processList(text, color)
    ...
    for _, line in ipairs(lines) do
        if line:find("^%d+x") then
            ...
            if count and model and currentHeroKey then
                ...
                table.insert(warbands[currentHeroKey].units, {count = tonumber(count), name = model})
            end
        else
            ...
            warbands[currentHeroKey] = { heroName = heroName, units = {} }
        end
    end
    ...
    for heroKey, data in pairs(warbands) do
        ...
        table.insert(spawnQueue, { name = data.heroName, count = 1, pos = heroPos })
        ...
        for _, entry in ipairs(data.units) do
            for i = 1, entry.count do
                ...
                table.insert(spawnQueue, { name = entry.name, count = 1, pos = modelPos })
                totalModels = totalModels + 1
    ...
    broadcastToColor("📢 " .. totalModels .. " Models | " .. breakPoint .. " Break | " .. quarterRemaining .. " Remaining", color, {0.2, 1, 0.2})
end
...
function spawnNextFromQueue(bag, queue, color)
        ...
        if entry_name == task_name then
            bag.takeObject({
                ...
                callback_function = function(original)
                    ...
                    Wait.time(function()
                        local clone = original.clone({ position = task.pos })
    ...
    broadcastToColor("⚠️ Could not find: " .. task.name, color, {1, 0.5, 0})
    spawnNextFromQueue(bag, queue, color)
end